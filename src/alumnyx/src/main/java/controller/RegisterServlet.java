...
public class RegisterServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String catagory = request.getParameter("branch");
		long prn_no = Long.parseLong(request.getParameter("prn_no"));
		String password = request.getParameter("password");
		
		if(role.equals("Alumini")) {
			AlumniData record = new AlumniData(name, uName, email, phoneno, graduationYear, catagory, prn_no, password);
		...
		{
			StudentData record = new StudentData(name, uName, email, phoneno, graduationYear, catagory, prn_no, password);
